...
func testDeleteDeployment(f *framework.Framework) {
	...
	d.Spec.Template.Spec.RuntimeClassName = &SecureRuntimeClassName
...
func testRollingUpdateDeployment(f *framework.Framework) {
	...
	rs.Spec.Template.Spec.RuntimeClassName = &SecureRuntimeClassName
...
func testRolloverDeployment(f *framework.Framework) {
	...
	rsReplicas := int32(1)
	rs := newRS(rsName, rsReplicas, rsPodLabels, WebserverImageName, WebserverImage, nil)
	rs.Spec.Template.Spec.RuntimeClassName = &SecureRuntimeClassName
	_, err := c.AppsV1().ReplicaSets(ns).Create(rs)
	framework.ExpectNoError(err)
	// Verify that the required pods have come up.
	...
	newDeployment.Spec.Template.Spec.RuntimeClassName = &SecureRuntimeClassName
